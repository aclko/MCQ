...
public class StudentNew extends JDialog {
	...
	public StudentNew() {
		...
		btnTakeImage.setBounds(559, 315, 145, 23);
		...
		label.setIcon(new ImageIcon(StudentNew.class.getResource("/images/back3.jpg")));
	...
	boolean checkStudentInTable() {
		...
		try {
			...
			if(rs.next()) {
				JOptionPane.showMessageDialog(null, "Student Already Registered","MCQ",JOptionPane.ERROR_MESSAGE);
				tstid.selectAll();
	...
	boolean check() {
		if(stid.length()==0) {
			JOptionPane.showMessageDialog(null, "Registration Number Not Provided","MCQ",JOptionPane.ERROR_MESSAGE);
			tstid.requestFocusInWindow();
			return false;
		}
		...
		if(m.isSelected())
			...
			JOptionPane.showMessageDialog(null, "Gender Not Selected","MCQ",JOptionPane.ERROR_MESSAGE);
	...
	void addStudent() {
		...
		String st="insert into Students(stid, stname, gender, stmobile, "
				+ "stemail, ststream, semester, stimage)values(?,?,?,?,?,?,?,?)";
		Connection con=MCQConnection.connect();
		try {
			...
			ps.executeUpdate();
			int more=JOptionPane.showConfirmDialog(this, "Student Registered for Quiz\n Register More","MCQ",JOptionPane.YES_NO_OPTION);
			if(more==0) {
				clear();
			}
			else {
